defmodule PhoenixChannelWorkshopWeb.Endpoint do
  use Sentry.PlugCapture
  use Phoenix.Endpoint, otp_app: :phoenix_channel_workshop

  alias Plug.Conn

  @plug_ssl Plug.SSL.init(rewrite_on: [:x_forwarded_proto])

  socket("/socket", PhoenixChannelWorkshopWeb.Socket)
  ...
  def init(_key, config) do
    if config[:load_from_system_env] do
      port = Application.get_env(:phoenix_channel_workshop, __MODULE__)[:http][:port] || raise "expected the PORT environment variable to be set"
  ...
  defp ping(%{request_path: "/ping"} = conn, _opts) do
    version = Application.get_env(:phoenix_channel_workshop, :version)
  ...
  defp canonical_host(conn, _opts) do
    opts = PlugCanonicalHost.init(canonical_host: Application.get_env(:phoenix_channel_workshop, :canonical_host))
  ...
  defp force_ssl(conn, _opts) do
    if Application.get_env(:phoenix_channel_workshop, :force_ssl) do
  ...
  defp cors(conn, _opts) do
    opts = Corsica.init(Application.get_env(:phoenix_channel_workshop, Corsica))

    Corsica.call(conn, opts)
  end

  defp basic_auth(conn, _opts) do
    basic_auth_config = Application.get_env(:phoenix_channel_workshop, :basic_auth)
